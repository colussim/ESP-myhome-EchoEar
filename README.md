...
- [x] Basic HA control
- [ ] Local wake word (ESP-SR or custom)
...
sequenceDiagram
    ...
    participant S as Satellite (ESP-VoCat)
    ...
    alt Wake detected
        B-->>S: JSON {category: WAKE, kiraactive: true, reply: "Oui ?"}
        S->>U: Play audio (id 7)
    else Not a wake
        B-->>S: Ignore / no action
    end
